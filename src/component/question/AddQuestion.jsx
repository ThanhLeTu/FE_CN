...
const handleUploadFile = async () => {
	...
	formData.append("subject", subject); // Lấy giá trị từ dropdown "Select Subject"
    formData.append("questionType", questionType); // Lấy giá trị từ dropdown "Question Type"


    try {
        setUploadStatus("Uploading..."); // Bắt đầu tải lên
        const response = await axios.post(
            "http://localhost:8080/api/upload", // API backend
            formData,
            {
                headers: {
        ...
        alert(response.data); // Hiển thị phản hồi thành công từ server
        setUploadStatus("File uploaded successfully!");
        fetchSubjects(); // Cập nhật danh sách chủ đề nếu cần
    } catch (error) {
...
////
					...
					<div className="card">
						<div className="card-header">
							...
							<div className="card-body">
								<form onSubmit={handleSubmit} className="p-2">
									<div className="mb-3">
										...
										<select
											...
											{Array.from(subjectOptions).map((subject,index) => (
								...
								<div className="mb-3">
    <label htmlFor="/upload" className="form-label text-info">
    ...
    {uploadStatus && <p className="mt-2 text-info">{uploadStatus}</p>}
</div>
